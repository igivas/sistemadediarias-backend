...
@injectable()
class UpdateDocumentoReassinarService {
  constructor(
  ...
  async execute({
    ...
    let documentoAtualizado = {} as Documento;

    const idsAssinaturas = documento.assinaturas
      ? documento.assinaturas.map(
          assinatura => assinatura.id_assinatura_documento,
        )
      : [];

    await getConnection().transaction(async manager => {
      const documentosRepository = manager.getRepository<Documento>(
        'documentos',
      );
      const assinaturasRepository = manager.getRepository<AssinaturaDocumento>(
        'assinaturas_documentos',
      );
      documentoAtualizado = documentosRepository.merge(
        documento,
        documentoObject,
      );
      await documentosRepository.save(documentoAtualizado);

      if (idsAssinaturas.length > 0) {
        await assinaturasRepository.delete(idsAssinaturas);
      }
    });

    delete documentoAtualizado.assinaturas;

    return { ...documentoAtualizado, url: fullPathLink };
  }
}

export default UpdateDocumentoReassinarService;
