...
class PessoasFisicasPmsRepository implements IPessoasFisicasPmsRepository {
  ...
  public async findUniformesByOpm(
    opm: string,
    page?: number,
    perPage?: number,
    ...
    const pessoa = await this.pessoaPmRepository.find({
      select: [
        'pm_codigo',
        'pm_apelido',
        'pm_numero',
  ...
  public async findUniformesByOpmPendentes(
    opm: string,
    page?: number,
    perPage?: number,
  ): Promise<PessoaFisicaPm[]> {
